...
struct
	...
	datatype instream =
		InStream of { state: streamState ref,
					  buffer: buffering,
					  rdr: reader }
			(* TODO: This assumes that the only operations we have are via the
			   reader functions.  We could have the alternative of direct access
			   to the underlying OS stream where possible, giving some
			   optimisations. *)
	...
	(* Extend a stream by adding a node to it showing that we
	   have read a vector. *)
	fun extendStream(InStream { state, buffer, rdr }, v: vector, start, length) =
	let
		val newState = ref ToRead
		val newStream = 
			InStream{state=newState, buffer=buffer, rdr=rdr}
	in
	...
	fun readVec (rdr as RD{readVec=SOME rv, name, ...}, request, caller) =
		(
			rv request handle exn => raise mapToIo(exn, name, caller)
		)
	| readVec(RD{name, ...}, _, caller) =
	...
	fun input (f as InStream {state = ref s,
		...
		|	HaveRead{vec, start, length, rest} =>(* Return what we read before. *)
				(vectorExtract(vec, start, length),
				 InStream{state=rest,rdr=rdr, buffer=buffer}) 
		|	ToRead =>
			case buffer of
				Unbuffered =>
				(
	...
	fun inputNList(f as InStream {state = ref s, rdr, buffer}, request) =
		case s of
			Truncated => (* Return empty list. *) ([], f)
		|	HaveRead{vec, start, length, rest} =>
				let
					val newStream = InStream{state=rest, rdr=rdr, buffer=buffer}
				in
					...
					in
						([result], InStream{state=ref r, rdr=rdr, buffer=buffer})
					end
					...
					let
						...
						val nextStream = 
							InStream {state=rest, rdr=rdr, buffer=buffer}
						val (rest, f') = inputNList (nextStream, request-length)
					in
	...
	fun inputN (f, request) =
		...
		let
			val (vlist, f') = inputNList(f, request)
		in
	...
	(* Can we input up to n characters without blocking?  Returns the
	   ...
	   a vector of size k.
	   *)
	fun canInput (f as InStream {state = r as ref s, ...}, request) =
	...
	fun setBufferMode(f as OutStream{buffType, buf, bufp, wrtr, ...}, newBuff) =
	...
	(* Internal function. Write a vector to the stream using the start and
	   length provided. *)
	fun outputVector(OutStream{isTerm=ref true, wrtr=WR{name, ...}, ...},_, _, _) =
		raise Io { name = name, function = "output", cause = ClosedStream }
	|   outputVector(f as OutStream{buffType, buf, bufp, wrtr, ...}, v, start, vecLen) =
	let
	...
	fun setPosOut(OutPos(f, p)) =
		case f of
			OutStream{wrtr=WR{name, setPos=SOME setPos, ...}, ...} =>
				(
				flushOut f;
				setPos p;
				f
				)
		|   OutStream{wrtr=WR{name, ...}, ...} =>
